...
/* PID values */
double BASE_SPEED = 230;
double KP = 2.40;
double KD = 110;
double KI = 0.1;
double NEAR_LINE_MODIFIER = 0.00;
...
/* Wi-Fi Credentials */
// const char* ssid = "CE303A.FIIR";
// const char* password = "#CE303A#FIIR!";

const char* ssid = "Dominic | Note 12 Pro+";
...
//   errorInt += error;
//   errorInt = constrain(errorInt, -400, 400); // Prevent integral windup
...
double PID(double input) {
  ...
  // double filteredErrorDiff = (error - lastError) * 0.5 + errorDiff * 0.5;
...
//                double* BASE_SPEED_ptr, double* KP_ptr, double* KD_ptr, double* KI_ptr, double* NEAR_LINE_MODIFIER_ptr, double* sensorWeights_ptr);
...
//     BASE_SPEED_ptr(BASE_SPEED_ptr), KP_ptr(KP_ptr), KD_ptr(KD_ptr), KI_ptr(KI_ptr), NEAR_LINE_MODIFIER_ptr(NEAR_LINE_MODIFIER_ptr),
...
//   html += "  var nearLineModifier = document.getElementById('nearLineModifier').value;";
...
//   html += "  xhttp.send('baseSpeed=' + baseSpeed + '&kp=' + kp + '&kd=' + kd + '&ki=' + ki + '&nearLineModifier=' + nearLineModifier + sensorWeights);";
...
//   html += "<p>Near Line Modifier: <input type='number' step='0.1' id='nearLineModifier' value='" + String(*NEAR_LINE_MODIFIER_ptr) + "'></p>";
...
//   if (server.hasArg("nearLineModifier")) *NEAR_LINE_MODIFIER_ptr = server.arg("nearLineModifier").toDouble();
...
// WebInterface webInterface(ssid, password, &BASE_SPEED, &KP, &KD, &KI, &NEAR_LINE_MODIFIER, lineSensor.sensorWeights);
...
void loop() {
  ...
  double right = BASE_SPEED + pid_output; //  = 5
